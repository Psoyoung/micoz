...
| **이력성 append-only (`his_*`, 이력성 `map_*`)** | **변경/삭제 금지** — 과거 사실(이력) 보존 | 포인트 이력(`his_point`), refresh token, 반품 라인(`dat_return_item`) 등 |
| **상태성 매핑 (현재 상태 표현 `map_*`)** | **교체 허용** — 현재 상태만 표현(보존 대상 아님) | 상품-라벨 매핑(`map_product_label`) |
| **단일행 설정** | **업데이트만**, 삭제 없음 | `mst_shipping`, `mst_user_grade` |

> **`map_*` 규칙 범위**: "변경/삭제 금지"는 **이력성 매핑**(과거에 일어난 사실을 보존해야 하는 매핑)에 한정한다.
> **상태성 매핑**(지금 붙어 있는 태그/관계처럼 "현재 상태"만 표현하고 과거 이력 의미가 없는 매핑)은 보존 대상이
> 아니므로 **교체 가능**하다. 대표 예: `map_product_label`(상품에 현재 붙은 라벨 집합) — 라벨 변경 시
> 차집합으로 빠진 매핑만 삭제, 새 매핑만 삽입(변경 없는 매핑은 미변경). 단순 추가/삭제 INSERT/DELETE 허용.
